...
- Enforce the published minimum station distance through one authoritative placement rule. Node use never grants ownership or blocks another station, while coordinate confirmation must serialize concurrent founding so two stations cannot commit invalid overlapping positions.
...
Persist each of the engagement's maximum three combat ticks as a separate idempotent resolution with its participant snapshot, preceding commands, losses, station effects, cargo transfers, and continuation state. After combat tick one or two, accept owner-authorized withdrawal commands until the next tick. Apply those withdrawals before resolving the next combat tick, preserve their secured cargo, and start their return; fleets without a withdrawal command remain committed. Apply the same transition to attackers and non-local defending fleets. Fleets arriving at that tick affect only the next unresolved combat tick when the engagement still continues. Removing all current defenders must not complete the engagement while the attacker retains a continuation-capable fleet and elects to use a remaining combat tick. End the engagement immediately after a combat tick that leaves no combat-capable attacking fleet, before a later combat tick when all remaining attackers withdraw, and unconditionally after combat tick three. Time-driven systems outside fleet activity do not need to share this hourly cadence unless their owning product rule requires it.
